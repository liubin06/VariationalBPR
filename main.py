...
def prior_plus(pos_scores, positem_id):
    ...
    batch_rarity = item_rarity[positem_id]  # [bs,M]
...
def prior_minus(neg_scores, negitem_id):
    ...
    batch_pop = item_pop[negitem_id]  # [bs,N]
...
def VarBPRExact(pos_score, neg_score, alpha, beta):
    ...
    :param pos_score: [bs * M]
    :param neg_score: [bs * N]
    :param alpha: [bs * M]
    :param beta:  [bs * N]
    :return:
    '''
...
def train(model, data_loader, train_optimizer):
    ...
    for batch in train_bar:
        ...
        # _______________________________________Fearture Exaction_____________________________________________
        ...
        pi_plus = prior_plus(pos_score, positem_id)  # [bs, M]
        pi_minus = prior_minus(neg_score, negitem_id)  # [bs, N]
        ...
        if args.loss == 'VarBPRPlugIn':
            loss = VarBPRPlugIn(pos_score, neg_score, alpha, beta)

        elif args.loss == 'VarBPRExact':
            loss = VarBPRExact(pos_score, neg_score, alpha, beta)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hop', default=2, type=int, help='Hop')

    parser.add_argument('--loss', default='VarBPRPlugIn', type=str,help='Choose Loss Function, choose from [VarBPRExact, VarBPRPlugIn]')
    ...
    parser.add_argument('--cpos', default=4, type=float, help='Positive scalling factor')
    parser.add_argument('--cneg', default=4, type=float, help='Negitive scalling factor')
    # build prior
    ...
    parser.add_argument('--hardnees_tau', default=0.5, type=float, help='Rarity of negative samples')
    ...
    ################################################# Build popularity related prior
    item_pop = torch.log1p(pop) / torch.log1p(pop.max())
    item_rarity = (1.0 - item_pop).clamp_min(1e-12)
    item_pop, item_rarity = item_pop.to(device), item_rarity.to(device)
    #################################################

    ################################################# Build item quality related prior
    good_quality = torch.sigmoid((quality - (quality.mean())) / 0.2)
    bad_quality = (1.0 - good_quality).clamp_min(1e-12)
    good_quality, bad_quality = good_quality.to(device), bad_quality.to(device)
    #################################################
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % 5 == 0:
            ...
            print(
                'Evaluation Epoch[{}/{}]: [Precision@{}: {:.4f}, Recall@{}: {:.4f}, F1@{}: {:.4f}, NDCG@{}: {:.4f}, APLT@{}: {:.4f}]'.format(
                    epoch, args.epochs, 5, result5[0], 5, result5[1], 5, result5[2], 5, result5[3], 5, result5[4]))
            result20 = evaluation.topk_eval(rating_matrix, 20, test_tensor.to(device), long_tail_mask=long_tail_mask)
            print(
                'Evaluation Epoch[{}/{}]: [Precision@{}: {:.4f}, Recall@{}: {:.4f}, F1@{}: {:.4f}, NDCG@{}: {:.4f}, APLT@{}: {:.4f}]'.format(
                    epoch, args.epochs, 20, result20[0], 20, result20[1], 20, result20[2], 20, result20[3], 20,
                    result20[4]))
            results.append(result5 + result20)
            print('\n')
    best_result, best_epoch = torch.tensor(results).max(dim=0)
    print('Best Results@Top-5: Precision:[{:.4f}], Recall:[{:.4f}], F1:[{:.4f}], NDCG:[{:.4f}], APLT:[{:.4f}]]'.format(best_result[0], best_result[1], best_result[2], best_result[3], best_result[4]))
    print('Best Results@Top-20: Precision:[{:.4f}], Recall:[{:.4f}], F1:[{:.4f}], NDCG:[{:.4f}], APLT:[{:.4f}]]'.format(best_result[5], best_result[6], best_result[7], best_result[8], best_result[9]))
    print('Best Epoch:[{}]'.format(best_epoch * 5))
    end = time.time()
    print(f"Total Runing time：{end - start:.6f} Second")
